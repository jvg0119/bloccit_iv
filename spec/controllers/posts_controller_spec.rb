...
# moderator Can create or update, but not delete, any post.
  context "moderator" do
    before do
      other_user.moderator!   # changed to moderator user
      create_session(other_user)
    end

    describe "GET #show" do   # same as any of the other users
      it "returns http success" do
        get :show, params: {topic_id: my_topic.id, id: my_post.id}
        expect(response).to have_http_status(:success)
      end
    ...
    describe "GET #new" do  # same as member user
      it "returns http success" do
        get :new, params: { topic_id: my_topic.id }
        expect(response).to have_http_status(:success)
      end
    ...
    describe "POST #create" do  # same as member user
      it "increases the number of Post by 1" do
        expect { post :create, params: { topic_id: my_topic.id, post: {title: RandomData.random_sentence, body: RandomData.random_paragraph} }
      }.to change{Post.count}.by(1)
      end
    ...
    describe "GET #edit" do   # same as admin user
      it "returns http success" do
        get :edit, params: { topic_id: my_topic.id, id: my_post.id }
        expect(response).to have_http_status(:success)
      end
    ...
    describe "PUT #update" do   # same as admin user
      it "updates post with expected attributes" do
        new_title = RandomData.random_sentence
        new_body = RandomData.random_paragraph
    ...
    describe "DELETE #destroy" do   # same as member user
      it "returns http redirect" do
        delete :destroy, params: { topic_id: my_topic.id, id: my_post.id }
        expect(response).to redirect_to([my_topic, my_post])
      end
